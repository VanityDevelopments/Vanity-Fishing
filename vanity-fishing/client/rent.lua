...
local function spawnBoat(boat)
    ...
    if closest then
        ...
        Framework.spawnVehicle(boat.model, coords, closest.spawn.w, function(vehicle)
            ...
            TriggerServerEvent('vanity_fishing:registerBoat', NetworkGetNetworkIdFromEntity(vehicle))
        end)
    end
end
...
local function rentVehicle(index)
    ...
    local success = lib.callback.await('vanity_fishing:rentVehicle', false, index)
...
local bind = lib.addKeybind({
    ...
    onReleased = function()
        ...
        local success = lib.callback.await('vanity_fishing:returnVehicle', false, netId)
