...
MySQL.ready(function()
    local data = MySQL.query.await('SELECT * FROM vanity_fishing')
...
local function save()
    local query = 'UPDATE vanity_fishing SET xp = ? WHERE user_identifier = ?'
    local parameters = {}
    local size = 0
...
local function createPlayer(identifier)
    ...
    MySQL.insert.await('INSERT INTO vanity_fishing (user_identifier, xp) VALUES(?, ?)', { identifier, levels[identifier] })
end

lib.callback.register('vanity_fishing:getLevel', function(source)
...
function AddPlayerLevel(player, amount)
    ...
    if math.floor(levels[identifier]) - level > 0 then
        TriggerClientEvent('vanity_fishing:showNotification', player.source, locale('unlocked_level'), 'success')
    end

    TriggerClientEvent('vanity_fishing:updateLevel', player.source, levels[identifier])
end
