...
for _, rod in ipairs(Config.fishingRods) do
    Framework.registerUsableItem(rod.name, function(source)
        ...
        local hasWater, currentZone = lib.callback.await('vanity_fishing:getCurrentZone', source)
        ...
        if not bait then
            TriggerClientEvent('vanity_fishing:showNotification', source, locale('no_bait'), 'error')
            busy[source] = nil
            return
        end
        ...
        local success = lib.callback.await('vanity_fishing:itemUsed', source, bait, Config.fish[fishName])
        ...
        elseif math.random(100) <= rod.breakChance then
            ...
            TriggerClientEvent('vanity_fishing:showNotification', source, locale('rod_broke'), 'error')
        end
